  ...
  return (
    <ToastContainer
      ...
      closeButton={({ closeToast }) => (
        <Close size={16} className={s.closeButton} onClick={closeToast} />
      )}
      hideProgressBar
      icon={Icon}
      position={'top-center'}
...
const Icon = ({ type }: IconProps) => {
  ...
  if (type === 'warning') {
    return <ErrorIcon color={'var(--color-danger-500)'} size={16} />
  }
  if (type === 'error') {
    return <Cancel color={'var(--color-accent-500)'} size={16} />
  }
